...
void InitBg(void)
{
	...
	//構造体の初期化処理
	g_aBg[0].pos = D3DXVECTOR3(BG_SCORE_WIDTH / 2, BG_SCORE_HEIGHT / 2, 2.0f);
	g_aBg[0].move = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	g_aBg[0].rot = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	g_aBg[0].nPatternAnim = 0;
	g_aBg[0].fAngle = atan2f(BG_SCORE_WIDTH, BG_SCORE_HEIGHT);
	g_aBg[0].fLength = sqrtf(BG_SCORE_WIDTH * BG_SCORE_WIDTH + BG_SCORE_HEIGHT * BG_SCORE_HEIGHT) / 2.0f;

	g_aBg[1].pos = D3DXVECTOR3(SCREEN_WIDTH / 2, BG_TIMER_HEIGHT / 2, 1.0f);
	g_aBg[1].move = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	g_aBg[1].rot = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	g_aBg[1].nPatternAnim = 0;
	g_aBg[1].fAngle = atan2f(BG_TIMER_WIDTH, BG_TIMER_HEIGHT);
	g_aBg[1].fLength = sqrtf(BG_TIMER_WIDTH * BG_TIMER_WIDTH + BG_TIMER_HEIGHT * BG_TIMER_HEIGHT) / 2.0f;
